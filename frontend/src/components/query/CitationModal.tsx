...
const CitationModal: React.FC<CitationModalProps> = ({ isOpen, onClose, citations }) => {
  ...
  const handleViewSource = (docId: string, docName: string, pageNumber: number) => {
    setPdfViewer({ docId, docName, pageNumber });
  };
  ...
  return (
    <>
      ...
      {/* PDF Viewer overlay — renders on top of the citation modal */}
      <PdfViewerModal
        isOpen={!!pdfViewer}
        onClose={() => setPdfViewer(null)}
        docId={pdfViewer?.docId ?? ''}
        docName={pdfViewer?.docName ?? ''}
        pageNumber={pdfViewer?.pageNumber ?? 1}
      />
    </>
  );
};
